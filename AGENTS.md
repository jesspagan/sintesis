...
Squash merge, title only (empty body — no `## Summary`/`## Test plan` boilerplate carried into the merge commit), and delete the branch on merge: `gh pr merge <number> --squash --subject "<title>" --body "" --delete-branch`.
