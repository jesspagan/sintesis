...
2. **Branch.** Never commit directly to `main`/`master`. If currently on `main`/`master`, create a feature branch first: `<type>/<slug>`, where `type` is `feat` (new capability — a skill, hook, memo covering genuinely new ground), `fix` (correcting something wrong), `docs` (memo/doc content, no process change), or `chore` (tooling/config/process). Pick whichever fits the actual change; when a commit mixes types, use the one that dominates. This is also hook-enforced, not just advisory — `.githooks/pre-commit` blocks the commit outright if run on `main`/`master`, since it's a "must not happen" case rather than a judgment call. This is a native git hook, not a Claude Code one — it fires from inside git's own commit machinery regardless of how the commit was invoked (see [hooks-and-harnesses](research/hooks-and-harnesses.md) §9 for why that replaced an earlier Claude-Code-hook approach). It only works if `core.hooksPath` is set to `.githooks` for this clone — a `SessionStart` hook (`.claude/hooks/configure-git-hooks-path.sh`) sets this automatically at the start of every Claude Code session, since `.git/hooks/` itself is never version-controlled.
